...
<div class="split right" style="width:55%; right:50px;">
  <div class="centered">
  <h1>Planters</h1>
  <?php 
	$connect = mysqli_connect("localhost", "root", "","roots");
	$sql = "SELECT * FROM planter";
	$result = mysqli_query($connect, $sql);
	if ($result->num_rows > 0) {
		echo "<table border=1><tr>";
		echo "<td>Name</td><td>Email</td><td>Birthday Date</td><td>Gender</td><td>Mobile Number</td><td>Address</td></tr>";
		while($row = $result->fetch_assoc()) {
			echo "<tr><td>" . $row["P_Fname"] . " ". $row["P_Lname"] . "</td>";
			echo "<td>" . $row["P_email"] ."</td>";
			echo "<td>" . $row["P_birthdate"] ."</td>";
			echo "<td>" . $row["P_Gender"] ."</td>";
			echo "<td>" . $row["P_phone"] ."</td>";
			echo "<td>" . $row["P_Address"] ."</td></tr>";
		}
		echo"</tr></table>";
	}
  ?>
