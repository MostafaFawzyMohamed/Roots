...
if($type=="Indoor"){
	$sql = "SELECT * FROM indoor WHERE I_id=". $id;
	...
	$sql = "SELECT * FROM outdoor WHERE O_id=". $id;
	$result = mysqli_query($connect, $sql);
	if ($result->num_rows > 0) {
		while($row = $result->fetch_assoc()) {
			$price=$row['O_price'];
...
?>
<div class="split right" style="width:55%; right:45px;">
  <div class="centered">
  <h1>Edit <?php echo $type; ?> Plants</h1>
  ...
  <table border=1 >
	...
	<tr>
		...
		<td><input type="number" name="quantity" value="<?php echo $quantity; ?>" required></td>
	</tr>
	
  </table>
  <br>
  <center><input type="submit" name="submit" class="button button4" value="Edit">
  </form>
</div>
</div>
